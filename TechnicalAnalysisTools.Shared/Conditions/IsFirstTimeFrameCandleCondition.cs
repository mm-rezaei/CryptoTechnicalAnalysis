...
namespace TechnicalAnalysisTools.Shared.Conditions
{
    ...
    public class IsFirstTimeFrameCandleCondition : CandleOperationConditionBase
    {
        public IsFirstTimeFrameCandleCondition(SymbolTypes symbol, TimeFrames timeFrame, int candleNumber, int nextMinutesCount) : base(symbol, timeFrame, candleNumber)
        {
        ...
        public override bool Calculate(OperationCandleRequestedHandler operationCandleRequested, TimeFrames? timeFrame = null, int? candleNumber = null)
        {
            ...
            if (candle != null)
            {
                ...
                result = IsThisMinuteCandleFirstTimeFrameCandle(dateTime, TimeFrame);
            }
            else
            {
                result = false;
